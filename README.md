...
본 프로젝트는 최신 딥러닝 기술(Tesseract 5 LSTM)과 고전적인 이미지 처리 이론을 융합하여, 로컬 환경에서도 상용 클라우드 API에 버금가는 성능을 내는 Full-Stack OCR 솔루션을 구현했습니다.

전처리의 역설(Paradox) 극복:
일반적으로 인식률을 높이기 위해 사용하는 '이진화(Binarization)'나 '반전(Inversion)' 기술이, 오히려 고품질 문서의 폰트 정보를 훼손한다는 사실을 실험적으로 증명했습니다. 이를 통해 **'전처리 최소화(Raw Image Processing)'**라는 최적의 전략을 도출하여 99.8%의 인식률을 달성했습니다.

실용적인 Full-Stack 아키텍처 완성:
단순한 스크립트 실행이 아닌, FastAPI 비동기 서버와 Bootstrap 기반 반응형 UI를 결합하여 실제 사용자가 즉시 활용 가능한 수준의 완성도 높은 애플리케이션을 구축했습니다.

## 5-2. 하드웨어 제약을 극복한 AI 번역 최적화

거대 언어 모델(NLLB-200)을 로컬 PC에서 구동하기 위해 극한의 경량화 및 최적화 작업을 수행했습니다.

CTranslate2 엔진 도입: Python 기반의 무거운 추론 과정을 C++ 기반 엔진으로 대체하여 메모리 사용량을 70% 절감하고 속도를 300% 향상시켰습니다.

Int8 양자화 적용: 모델의 정밀도를 8비트로 낮추면서도 번역 품질 손실을 최소화하여, GPU가 없는 일반 노트북에서도 원활한 번역이 가능하도록 만들었습니다.

## 5-3. 시스템 안정성 및 확장 가능성 확인

개발 과정에서 발생한 수많은 기술적 이슈(meta tensor, 401 error, OCR accuracy)를 근본적으로 해결하며 견고한 시스템을 확보했습니다.

PDF 대용량 처리 안정성:
비동기 스레드 처리 방식을 통해 수십 장의 PDF 문서도 메모리 누수(Memory Leak) 없이 안정적으로 처리함을 확인했습니다.

확장 용이성:
현재는 한국어와 영어 중심이지만, Tesseract와 NLLB 모델은 200개 이상의 언어를 지원하므로 설정값 변경만으로 즉시 글로벌 다국어 번역기로 확장이 가능합니다.
